...
- 2026-09-24 — Added the verification-pending deterministic/statistical operational Health Engine with versioned configurable thresholds, rolling median/MAD baselines, bounded cluster-wide Prometheus evidence, structured anomalies, multi-replica-safe scheduled snapshots, and numeric change/health deltas.
- 2026-09-24 — Added the verification-pending Operations foundation with durable sanitized change journaling, versioned persisted Health Snapshots/reports, change-to-health timeline correlation, journaled configuration/security/DLQ recovery actions, and capacity-tooling scenario markers without introducing an LLM runtime dependency.
- 2026-09-24 — Prioritized operational health intelligence with new stable `OBSERVABILITY.HEALTH_INTELLIGENCE`, `OPERATIONS.CHANGE_JOURNAL`, and `OBSERVABILITY.ASSISTED_INVESTIGATION` capabilities; added the active specification for persisted Health Reports, change/health correlation, lightweight anomaly detection, manual LLM export, bounded read-only agentic investigation, and application-owned alert policy.
