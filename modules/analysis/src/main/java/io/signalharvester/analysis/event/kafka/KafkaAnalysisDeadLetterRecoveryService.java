...
/** Reads Analysis DLQ records by position and reprocesses their original bytes without republishing shared events. */
@Singleton
public final class KafkaAnalysisDeadLetterRecoveryService implements DeadLetterRecovery {

    private static final Logger LOGGER = LoggerFactory.getLogger(KafkaAnalysisDeadLetterRecoveryService.class);
    ...
    public KafkaAnalysisDeadLetterRecoveryService(
        ...
        this.allowedSourceTopics = Set.of(requireNonBlank(rawItemTopic, "rawItemTopic"));
    ...
    public Inspection replay(int deadLetterPartition, long deadLetterOffset, String expectedDeadLetterId) {
        return withPermit(() -> {
            ...
            try {
                rawItem = decoder.decode(sourceKey(loaded.event()), loaded.event().getSourcePayload().toByteArray());
                ...
                throw new DeadLetterRecoveryException(
                        ...
                        "Original Analysis source record is still invalid under the current decoder",
                        invalidSourceRecord);
            }
            try {
                ...
                throw new DeadLetterRecoveryException(
                        ...
                        "Analysis replay failed under the current application state",
                        replayFailure);
            }
            LOGGER.info(
                    "Replayed Analysis dead letter {} from {}-{}@{} through the owning application path",
    ...
    private LoadedDeadLetter load(int deadLetterPartition, long deadLetterOffset) {
        ...
        try (KafkaConsumer<String, byte[]> consumer = new KafkaConsumer<>(consumerProperties())) {
            ...
            throw new DeadLetterRecoveryException(
                    ...
                    "Unable to read the Analysis dead-letter topic",
                    kafkaFailure);
        }
    }
    ...
    private LoadedDeadLetter validate(ConsumerRecord<String, byte[]> record, String deadLetterTopic) {
        if (record.value() == null) {
            throw invalidRecord("Analysis dead-letter record has no value", null);
        }
        final DeadLetterEvent event;
        try {
            event = DeadLetterEvent.parseFrom(record.value());
        } catch (InvalidProtocolBufferException exception) {
            throw invalidRecord("Analysis dead-letter record is not a valid DeadLetterEvent", exception);
        }
        ...
        if (!event.getDeadLetterId().equals(canonicalId) || !Objects.equals(record.key(), event.getDeadLetterId())) {
            throw invalidRecord("Analysis dead-letter identity does not match its source metadata", null);
        }
        if (!CONSUMER_NAME.equals(event.getConsumer())) {
            throw invalidRecord("Dead-letter record does not belong to the Analysis consumer", null);
        }
        if (!consumerGroup.equals(event.getConsumerGroup())) {
            throw invalidRecord("Dead-letter record does not belong to the configured Analysis consumer group", null);
        }
        if (!allowedSourceTopics.contains(event.getSourceTopic())) {
            throw invalidRecord("Analysis dead-letter source topic is not an allowed Analysis input topic", null);
        }
        if (event.getSourcePartition() < 0 || event.getSourceOffset() < 0 || event.getAttempts() < 1) {
            throw invalidRecord("Analysis dead-letter source metadata is invalid", null);
        }
    ...
    private Map<String, Object> consumerProperties() {
        ...
        properties.put(ConsumerConfig.CLIENT_ID_CONFIG, "signalharvester-analysis-dead-letter-recovery");
        return properties;
    }
    ...
    private <T> T withPermit(Supplier<T> work) {
        if (!permits.tryAcquire()) {
            throw new DeadLetterRecoveryException(
                    ...
                    "Analysis dead-letter recovery is already at its configured concurrency limit");
        }
        try {
            return work.get();
        } finally {
