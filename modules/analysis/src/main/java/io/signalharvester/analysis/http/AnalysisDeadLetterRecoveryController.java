...
public class AnalysisDeadLetterRecoveryController {
    private static final Logger LOG = LoggerFactory.getLogger(AnalysisDeadLetterRecoveryController.class);

    private final DeadLetterRecovery recovery;
    private final OperationalChangeJournal changeJournal;

    public AnalysisDeadLetterRecoveryController(DeadLetterRecovery recovery, OperationalChangeJournal changeJournal) {
    ...
    /** Reprocesses one explicitly confirmed DLQ record through the Analysis-owned application path. */
    ...
    private void recordReplayBestEffort(DeadLetterRecovery.Inspection inspection, HttpRequest<?> request) {
        try {
            ...
            changeJournal.record(new OperationalChangeRequest(
                    ...
                    "analysis:" + inspection.deadLetterPartition() + ":" + inspection.deadLetterOffset(),
                    Map.of(),
                    Map.of(
                            "action", "replayed",
            ...
            LOG.warn(
                    "Dead-letter replay succeeded but operational change journaling failed for analysis partition={} offset={}",
