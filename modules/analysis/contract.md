...
- each claimed outbox row renews its exact-token lease immediately before Kafka publication only while the persisted lease is still live; an expired former owner or stale token holder that can no longer renew must not publish the row;
- pre-publication renewal completes before Kafka I/O and prevents local batch queueing from consuming a later row's ownership window or resurrecting expired ownership; it does not provide distributed exactly-once delivery or guarantee that one Kafka send cannot outlive the renewed lease;
...
- ordinary publication-failure retry metadata may be written only while the exact-token outbox lease is still live; an expired former owner must leave the row immediately reclaimable instead of pushing recovery into a new backoff window;
