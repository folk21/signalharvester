...
Administrative identity management is exposed at `/api/v1/admin/users`. Responses never include password hashes or JWT material. Administrative updates are serialized at the security persistence boundary and reject disabling or removing `ADMIN` from the last enabled administrator. Authentication outcomes, administrative identity changes, and denied API requests are logged without password, hash, signing-key, or token contents. The first administrator may be bootstrapped from deployment-provided username/password settings when security is enabled and no enabled `ADMIN` exists.
