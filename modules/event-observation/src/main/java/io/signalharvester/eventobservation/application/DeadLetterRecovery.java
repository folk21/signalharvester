...
/** Internal operator boundary for inspecting and replaying Event Observation dead-letter records. */
public interface DeadLetterRecovery {

    /** Loads one Event Observation dead-letter record by its Kafka DLQ position. */
    Inspection inspect(int deadLetterPartition, long deadLetterOffset);

    /** Reprocesses one confirmed dead-letter record through the Event Observation-owned application path. */
