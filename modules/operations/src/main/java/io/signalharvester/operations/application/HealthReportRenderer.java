...
public final class HealthReportRenderer {
    ...
    /** Renders one snapshot and its referenced recent changes without embedding raw telemetry. */
    public String render(HealthSnapshot snapshot, List<OperationalChangeRecord> recentChanges) {
        ...
        report.append("## Anomaly candidates\n\n");
        if (snapshot.anomalyCandidates().isEmpty()) {
            report.append("- None produced by the current policy.\n\n");
        } else {
            snapshot.anomalyCandidates().forEach(value -> report.append("- `").append(inline(value)).append("`\n"));
            report.append('\n');
        }
