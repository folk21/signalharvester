...
public class ResultsDeadLetterRecoveryController {
    private static final Logger LOG = LoggerFactory.getLogger(ResultsDeadLetterRecoveryController.class);

    private final DeadLetterRecovery recovery;
    private final OperationalChangeJournal changeJournal;

    public ResultsDeadLetterRecoveryController(DeadLetterRecovery recovery, OperationalChangeJournal changeJournal) {
    ...
    /** Reprocesses one explicitly confirmed DLQ record through the Results-owned application path. */
    ...
    private void recordReplayBestEffort(DeadLetterRecovery.Inspection inspection, HttpRequest<?> request) {
        try {
            ...
            changeJournal.record(new OperationalChangeRequest(
                    ...
                    "results:" + inspection.deadLetterPartition() + ":" + inspection.deadLetterOffset(),
                    Map.of(),
                    Map.of(
                            "action", "replayed",
            ...
            LOG.warn(
                    "Dead-letter replay succeeded but operational change journaling failed for results partition={} offset={}",
