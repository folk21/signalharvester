...
Verification-pending assisted-investigation Stage 3 remains inside Operations: a bounded `health-analysis-v1` package wraps the sanitized Health Report with an explicit telemetry-is-untrusted instruction and stable evidence references; validated Incident Assessments are persisted separately and cannot modify the Health Snapshot status/score. Manual import is always available after package export. Optional model invocation uses a provider-neutral `IncidentAnalyst` boundary; the first adapter is OpenAI-compatible over JDK HTTP, disabled by default, and performs no tool calls or automatic triggers. Provider network I/O occurs outside database transactions.
