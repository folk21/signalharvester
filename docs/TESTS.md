...
- `RawItemProcessingPostgresIntegrationTest` for focused new/irrelevant/duplicate processing, independent cross-profile acceptance, atomic deduplication/outbox commit, persisted outbox trace context, outbox retry metadata, identical-byte republication after a Kafka-ack/published-marker failure, and claim/counter rollback when outbox staging fails;
...
Security unit tests protect password hashing and SignalHarvester-specific JWT subject/role claims. The PostgreSQL/server integration suite covers first-ADMIN bootstrap, real login-token signature/issuer/audience/subject/role claims, HttpOnly JWT plus readable CSRF cookies, invalid JWT signature/expiry/issuer/audience validation, explicit USER/VIEWER/ADMIN/BOT roles, CSRF mutation rejection, allowed and rejected credentialed CORS origins, disablement semantics, logout, sequential/concurrent last-enabled-ADMIN protection, and ADMIN user management without exposing password hashes. The cross-module application test proves `VIEWER` access to Results list/detail/SSE, denies VIEWER diagnostic/admin access, proves `ADMIN` without `VIEWER` does not inherit Results access, verifies role changes apply to newly issued credentials while old JWTs remain stateless, verifies USER/BOT-only principals receive no business capability, and keeps health/Prometheus anonymous under security. Routine acceptance still requires `./run_checks.sh`.
