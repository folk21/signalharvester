...
spec_status: active
current_focus: subspecs/backend-processing-flow-reconstruction.md
---
...
The current technical focus is processing-flow reconstruction under `subspecs/backend-processing-flow-reconstruction.md`. Bounded technical event observation and Results-owned SSE live delivery were accepted after developer verification runs and are archived. Diagnostic source testing plus configuration-driven REST/JSON and HTML extraction were also accepted and archived with the earlier completed configuration, collection, analysis, Results, scheduling, and operational-administration slices. The Kafka/Protocol Buffers contract model in `subspecs/backend-event-contracts.md` and established module-boundary rules in `subspecs/backend-project-structure.md` remain active supporting specifications.
