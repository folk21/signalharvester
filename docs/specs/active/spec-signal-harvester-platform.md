...
spec_status: active
current_focus: subspecs/backend-event-observation.md
---
...
The current technical focus is bounded technical event observation under `subspecs/backend-event-observation.md`. Results-owned SSE live delivery was accepted after the developer verification run and is archived. Diagnostic source testing plus configuration-driven REST/JSON and HTML extraction were accepted after the developer verification run and are archived with the earlier completed configuration, collection, analysis, Results, scheduling, and operational-administration slices. The Kafka/Protocol Buffers contract model in `subspecs/backend-event-contracts.md` and established module-boundary rules in `subspecs/backend-project-structure.md` remain active supporting specifications.
