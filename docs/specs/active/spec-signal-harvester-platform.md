...
spec_status: active
current_focus: subspecs/backend-reliability-failure-handling.md
---
...
The current technical focus is bounded Kafka consumer retry, poison-record handling, and dead-letter delivery under `subspecs/backend-reliability-failure-handling.md`. Processing-flow reconstruction is implemented and remains active only while developer acceptance verification is pending. Bounded technical event observation and Results-owned SSE live delivery were accepted after developer verification runs and are archived. Diagnostic source testing plus configuration-driven REST/JSON and HTML extraction were also accepted and archived with the earlier completed configuration, collection, analysis, Results, scheduling, and operational-administration slices. The Kafka/Protocol Buffers contract model in `subspecs/backend-event-contracts.md`, established module-boundary rules in `subspecs/backend-project-structure.md`, and the future authentication/authorization design in `subspecs/backend-authentication-authorization.md` remain active supporting specifications.
