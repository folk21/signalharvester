...
4. **Read-only agentic investigation.** Add bounded Prometheus/Loki/Tempo/change-history/capacity tools, investigation budgets, sanitization, and explicit provider tool turns.
5. **Automatic trigger policy.** Add multi-replica-safe event/periodic invocation, cooldown, and deduplication without holding database transactions during provider calls.
6. **Alert integration and model evaluation.** Combine deterministic health persistence with optional structured LLM assessment for human-attention alerts; only then evaluate whether custom ML/neural models add measurable value.
