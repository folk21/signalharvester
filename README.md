...
- verification-pending Kubernetes backend/infrastructure deployment with PostgreSQL, Redpanda, Prometheus, Loki, Tempo, Grafana Alloy, kube-state-metrics, Grafana, and a separately owned frontend workload boundary;
- verification-pending Kubernetes resilience acceptance tooling for restart, persistence outage, retry/DLQ, Kafka lag, outbox recovery, scheduler leases, authorization, and telemetry evidence.

External-source access security and backend authentication/authorization are accepted. The default host-run local profile remains the explicitly trusted unauthenticated compatibility mode until the frontend is migrated, while the `security` environment enables authentication/RBAC and restrictive outbound-source policy. Kubernetes backend/infrastructure manifests and the initial observability stack are implemented and verification-pending. A live resilience harness now exercises controlled restart, lag, retry/DLQ, outbox, scheduler, authorization, PostgreSQL/Redpanda recovery, logs, traces, and metrics on that cluster. Full umbrella Kubernetes acceptance still requires a real `signalharvester-web` image from the companion repository. The Analysis authoritative-state/Kafka consistency gap is addressed by the accepted transactional outbox. Repository-owned Docker Compose remains the lightweight local PostgreSQL/Kafka development path.
